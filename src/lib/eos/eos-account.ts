...
*/
export const buildNewAccountTransaction = (newAccountName: string, creatorName: string, creatorPrvKey: string, chainId: string, userKey: string, backupKey: string, serviceKey: string, refBlockNum:number,refBlockPrefix:number, now?: moment.Moment): Promise<any> => {
  const privateKeys = [creatorPrvKey];
  const creatorPublicKey = PrivateKey.fromString(creatorPrvKey).toPublic().toString();
